...
# ── Page config ──────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="DeepMind · AI Research Agent",
    page_icon="🔬",
    layout="wide",
...
if st.session_state.running and not st.session_state.done:
    ...
    with st.spinner("🔍 Search Agent is working…"):
        results["search"] = web_search_tool.invoke({
            "query": topic_val
        })
    ...
    # Step 2: Reader
    with st.spinner("📄 Reader Agent is processing content…"):
...
"""
        ...
        results["writer"] = writer_chain_.invoke({
            "topic": topic_val,
...
st.markdown("""
<div class="notice">
    DeepMind · Powered by LangChain multi-agent pipeline · Built with Streamlit
</div>
""", unsafe_allow_html=True)